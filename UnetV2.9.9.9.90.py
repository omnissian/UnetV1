...
path_learn_in="/home/std_11/MazurM/UnetV1/DataSet1/db_small_3/train/in/"
path_learn_targets="/home/std_11/MazurM/UnetV1/DataSet1/db_small_3/train/mask/"
path_vaild_in="/home/std_11/MazurM/UnetV1/DataSet1/db_small_3/valid/in/"
path_vaild_targets="/home/std_11/MazurM/UnetV1/DataSet1/db_small_3/valid/mask/"
#------------------------
...
# #---------------------------------------------------------------------------------
# path_learn_in="/home/std_11/MazurM/UnetV1/DataSet1/db_small_1/train/in/"
# path_learn_targets="/home/std_11/MazurM/UnetV1/DataSet1/db_small_1/train/mask/"
# path_vaild_in="/home/std_11/MazurM/UnetV1/DataSet1/db_small_1/valid/in/"
# path_vaild_targets="/home/std_11/MazurM/UnetV1/DataSet1/db_small_1/valid/mask/"
#-------------------------------------------------------------------------------------------
...
file="epoch_175_err_0.00734_GoogleUnet1v1Params.pth" # change the name of the file that you want to LOAD from
...
data_set=DatasetCustom(1) # argument==1 for train
data_train=data.DataLoader(data_set, batch_size=batch_size, num_workers = 4)
learn_rate=3e-3
net = Model()
net=net.cuda() # <-------------------
momentum=0.9
...
str_a=str(input()) # run with sudo python3 !!!!!
# pdb.set_trace()
if(str_a=="Y" or str_a=="y" or str_a=='Y' or str_a=='y'):
...
if(str_a=="Y" or str_a=="y" or str_a=='Y' or str_a=='y'):
    ...
    for ep in range(1,amount_epochs+1):
        ...
        for i,(img,label) in enumerate(data_train):
            ...
            label=label.cuda()
            output=net(img) # should be size torch.Size([BATCHSIZE, numclasses, Height,WIdht])
            # pdb.set_trace()
            ...
            eq_in_batch=torch.sum(torch.eq(predicted,label))
            elements_in_batch=256**2 * batch_size
            # print("elements_in_batch = ",elements_in_batch)
            # print("eq_in_batch=",eq_in_batch.item())
            # print("elements_in_batch=",elements_in_batch)
            eq_in_batch=eq_in_batch.item()
            accur_in_pix=eq_in_batch/elements_in_batch
            # print("accuracy in PIXels= ",accur_in_pix)
            pix_accur+=accur_in_pix
            # pdb.set_trace()
            # #-----------------------------------
            # print(error.data)
            # print("breakpoint")
           # #----------------------------------for check whats an images----------------------
            ...
            # ### inter=input()
            # #----------------------------------for check whats an images----------------------
        print("PIX error= ",(1-pix_accur/counter))
        print("AVG Error=",(err/counter))
        er_sv=(err*1.0)/(counter*1.0)
        # print("now parameters will be saved")
        #--------------------------------------------------------------
        if (not(ep % step_save)):
            prefix="epoch_"+str(ep)+"_err_"+str(er_sv)+"_"
        ...
        ##------------------------------------------------------------
        # if (ep == 10 or ep == 20 or ep == 30 or ep == 40):
        #     print(">>------------PARAMETERS SAVED------------<<")
        #     save_folder = "/home/std_11/MazurM/UnetV1/Parameters/"
        #     # save_folder="/storage/MazurM/Task1/SavedParameters/UnetV1/polygon/"
        #     file_saved_parameters = "epoch" + str(ep) + "err_" + str(err) + "_Unet1v1Params.pth"
        #     torch.save(net.state_dict(), os.path.join(save_folder, file_saved_parameters))
...
data_valid=data.DataLoader(data_set, batch_size=batch_size_valid)
with torch.no_grad():
    for i,(img,label) in enumerate(data_valid):
        ...
        ##---------------------------------------------------------------
        img_pred=np.asarray(predicted.cpu())
        for index in range (batch_size_valid):
            ...
            print("check your saved parameters")
            # pdb.set_trace()
            #-----------------trying to save into correct colours below-----------

            #-----------------trying to save into correct colours above-----------
            img_pred_draw=torch.from_numpy(img_pred_draw)
            img_pred_draw=img_pred_draw.type(torch.FloatTensor)
            img_pred_draw=transforms.ToPILImage()(img_pred_draw)
            # img_pred_draw.save("/home/std_11/MazurM/UnetV1/PredictedImages/predict.tiff")
            # img_pred_draw.save("/home/std_11/MazurM/UnetV1/PredictedImages/"+str(index)+"V2predict.tiff")
            img_pred_draw.save("/home/std_11/MazurM/UnetV1/PredictedImages/predict_db_small_1/"+str(i*batch_size_valid+index)+"V2predict.tiff")
            #------------label---------------
            label_out = label.type(torch.FloatTensor)
            label_out=label_out.data
            label_out=label_out.cpu()
            ...
            wtf.save("/home/std_11/MazurM/UnetV1/PredictedImages/predict_db_small_1/"+str(i*batch_size_valid+index)+"V2label.tiff")
            #-----------img----------------------
            #-----------------trying to save into correct colours below-----------
            # pdb.set_trace()
            # img_in=img.cpu()
            # img_in=img_in[index]
            # img_in=img_in.permute(1,2,0)
            # img_in=np.array(img_in)
            # plt.imsave("/home/std_11/MazurM/UnetV1/PredictedImages/predict_db_small_1/"+str(index)+"V2label.tiff",img_in)
            #-------------------------------------------------
            img_in=img.cpu()
            img_in=img_in[index]
            img_in=img_in.permute(1,2,0)
            # img_in=img_in.type(torch.int32)
            img_in=img_in.type(torch.uint8)
            img_in=np.array(img_in)
            # pdb.set_trace()
            # img_in=Image.fromarray(img_in)
            plt.imsave("/home/std_11/MazurM/UnetV1/PredictedImages/predict_db_small_1/"+str(i*batch_size_valid+index)+"V2Input.tiff",img_in)
            ...
            # img_in.save("/home/std_11/MazurM/UnetV1/PredictedImages/predict_db_small_1/"+str(i*batch_size_valid+index)+"V2Photo.tiff")
            # # pdb.set_trace()
