...
class DatasetCustom(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        img=torch.from_numpy(img)
        print(type(img))
        print(img.dtype) # torch.uint8 for image
        img=img.permute(2,0,1)
        ### -------------
        # img=img.type(torch.FloatTensor)
        # print(img.requires_grad)# requires_grad=false
        print(img.dtype) # torch.uint8 for image
        print(img)
        pdb.set_trace()
        print("----------img.dtype --BEFORE------",img.dtype) # torch.uint8 for image
        img=img.type(torch.float64)
        print("----------img.dtype --AFTER------",img.dtype)
        img.requires_grad=True
        print("---img.requires_grad ---->>--",img.requires_grad)
        img=torch.tensor(img,dtype=torch.float64,requires_grad=True)
        ...
        ### -------------
        # return img.type(torch.FloatTensor), label.type(torch.FloatTensor)
        # return img, label.type(torch.FloatTensor)
    def __len__(self):
        return len(names_valid)
