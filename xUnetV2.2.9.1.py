...
# #--------------------------------------------------------------------
# train in
# /home/std_11/MazurM/DB_cut_images/DB_cut/google
# train label
# /home/std_11/MazurM/DB_cut_images/DB_cut/all_mask
#
# valid in
# /home/std_11/MazurM/DB_cut_images/DB_cut/Valid/GoogleIn
# valid label
# /home/std_11/MazurM/DB_cut_images/DB_cut/Valid/Labels
...
batch_size=8 #always in 2^N compatible. Warning: Dont forget to write a batch size to saved parameters filenames!!!
data_set=DatasetCustom("train")
data_train=data.DataLoader(data_set, batch_size=batch_size)

data_set_v=DatasetCustom("valid")
data_valid=data.DataLoader(data_set_v,batch_size=batch_size)
learn_rate=1e-3
net = Model()
net=net.cuda()
# pdb.set_trace()
...
file="epoch_1400_err_0.01950030028820038_Unet1v1Params.pth" # change the name of the file that you want to load from
...
if(str_a=="Y" or str_a=="y"):
    ...
    # pdb.set_trace()
    err=0.0
    for ep in range(0,amount_epochs+1):
        error_sum=0.0
        exemplars=0
        if (not(ep % step_save)):
            ...
            file_saved_parameters = prefix+"Unet1v1Params.pth"
        ...
        #             images_valid.append(np.zeros((3,width,height)))
        #             label_valid.append(np.zeros((3,width,height)))
        #             for w in range (width):
        #                 for h in range(height):
        #                     if(img_pred[index,w,h]):
        #                         images_valid[index][0,w,h]=255
        #                         images_valid[index][1,w,h]=255
        #                         images_valid[index][2,w,h]=255
        #                     else:
        #                         images_valid[index][0,w,h]=250
        #                         images_valid[index][1,w,h]=134
        #                         images_valid[index][2,w,h]=1
        #             images_valid[index] = torch.from_numpy(images_valid[index])
        #             images_valid[index] = images_valid[index].type(torch.FloatTensor)
        #             images_valid[index] = transforms.ToPILImage()(images_valid[index])
        #             images_valid[index].save("/home/std_11/MazurM/UnetV1/PredictedImages/" + str(i) + "V2predict.tiff")
        #
        #             wtf=label[index]
        #             wtf=wtf.data
        #             wtf=wtf.cpu()
        #             pdb.set_trace()
        #             label_valid[index]=transforms.ToPILImage()(wtf) ###Input type float64 is not supported
        #             label_valid[index].save("/home/std_11/MazurM/UnetV1/PredictedImages/" + str(i) + "V2label.tiff")
        #
        #             # label_valid[index] = wtf.type(torch.FloatTensor)
        #             # label_valid[index]=
        #
        #
        #             # label_valid[index] = label[index].type(torch.FloatTensor)
        #             # label_valid[index] = label[index].data
        #             # label_valid[index] = label[index].cpu()
        #             # label_valid[index]=transforms.ToPILImage()(label_valid[index])
        #             # label_valid[index].save("/home/std_11/MazurM/UnetV1/PredictedImages/" + str(i) + "V2label.tiff")
        #
        #             #-------------------------------------------------------------
        #             # # pdb.set_trace()
        ...
        #             # img_pred_draw.save("/home/std_11/MazurM/UnetV1/PredictedImages/" + str(i) + "V2predict.tiff")
        ...
        print("epoch ",ep," error on train set is=",error_sum/exemplars)
...
# data_set=DatasetCustom("train")
# data_valid=data.DataLoader(data_set, batch_size=batch_size)
with torch.no_grad():
    for i,(img,label) in enumerate(data_valid):
        correct=0
        img=img.cuda()
        ...
        for w in range(width):
            for h in range(height):
                if(img_pred[0,w,h]):
                    img_pred_draw[0,w,h]=255
                    img_pred_draw[1,w,h]=255
                    img_pred_draw[1,w,h]=255
                else:
                    img_pred_draw[0,w,h]=250
                    img_pred_draw[1,w,h]=134
                    img_pred_draw[1,w,h]=1
