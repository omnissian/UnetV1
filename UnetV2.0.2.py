...
for epoch in range(amount_epochs):
    with torch.no_grad():
        ...
        accuracy=(true_predicted_pixels/amount_all_pixels)*100.0
    ...
    for i,(data_in,label) in enumerate(data_train):
        ...
        # error=loss(predict,label)
        error=loss(predict[:,0,:,:],label) #must be 4 D tensors both, should add one more dimension to "label" tensor
        optimizer.zero_grad()
        error.backward()
        optimizer.step()
